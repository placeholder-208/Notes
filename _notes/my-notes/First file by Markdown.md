...
- 您可以使用一对双等号\=\=表明其中内容应被渲染高亮背景。eg:渲染前 \=\=示例文本\=\= 渲染后 ==示例文本==

### 上下标与注解

使用一对`^`表示区域内的文本应被渲染为前一文本的上标。使用一对`~`表示区域内的文本应被渲染为前一文本的下标。eg:文本1^上标^ 文本2~下标~

使用`^[]`表示对前一文本进行注解，中括号内文字为具体注解内容，点击注释上标将快速跳转到注释文字，，点击注释文字后的回车将快速回到该注释文字所注释的内容。eg:渲染前 文本\^[注释文本1]
...
渲染前:<br>\$\$y=x+1 \space  \space y=x^2\$\$

渲染后：
$$
y=x+1 \space \space y=x+2
$$
...
- 其他
   ...
   - 下花括号 \underbrace{}_{} $\underbrace{x_1,x_2,\cdots x_n}_{n}$
