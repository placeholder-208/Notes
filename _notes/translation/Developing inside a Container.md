...
* 如果你在本地主机上同时使用Widnows和container对同一仓库进行编辑，确保你已统一行尾符号。阅读文档[提示和技巧]()了解更多。
* 如果你使用了Git证书管理器管理clone，你的container也应该有权限获取你的证书。如果你使用了SSH密钥方式连接，你也应该选择共享密钥。阅读文档[给你的container分享Git认证]()了解更多。
...
本文档包括三种快速开始的方式，我们推荐你从中选择最符合您工作流和兴趣的方式：
This document includes 3 quick starts - we recommend starting with the one that fits your workflow and interests the best:

1.仅仅想要尝试一个dev container例子，阅读本文档第一种方式
2.想要为本地已存在的克隆仓库添加dev container，阅读本文档第二种方式
3.想要为仓库创建一个独立的副本，用于在不影响本地工作的情况下处理PR或分支，阅读本文档第三种方式
